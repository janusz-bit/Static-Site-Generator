...
class TestTextNode(unittest.TestCase):
    ...
    def test_split_nodes_delimiter_at_start(self):
        node = TextNode("`code block` then text", TextType.TEXT)
        new_nodes = split_nodes_delimiter([node], "`", TextType.CODE)
        self.assertEqual(
            new_nodes,
            [
    ...
    def test_split_nodes_delimiter_at_end(self):
        node = TextNode("text then `code block` status", TextType.TEXT)
        new_nodes = split_nodes_delimiter([node], "`", TextType.CODE)
        self.assertEqual(
            new_nodes,
            [
                ...
                TextNode("code block", TextType.CODE),
                TextNode(" status", TextType.TEXT),
            ],
        )

    def test_split_nodes_delimiter_bold_double(self):
        node = TextNode("word **bold** word **bold**", TextType.TEXT)
        new_nodes = split_nodes_delimiter([node], "**", TextType.BOLD)
        self.assertEqual(
            new_nodes,
            [
